...
<div class="wrap">
  ...
  <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
    ...
    <input type="hidden" name="created_at" value="<?php echo isset( $data['created_at'] ) ? ( $data['created_at'] ) : ''; ?>">
    <input type="hidden" name="subscription_id" value="<?php echo isset( $data['id'] ) ? absint( $data['id'] ) : 0; ?>">

    <table class="form-table" role="presentation">
      <tr>
        <th scope="row"><label for="plan_name">Plan Name *</label></th>
        <td><input type="text" name="plan_name" id="plan_name" class="regular-text" required value="<?php echo isset( $data['plan_name'] ) ? esc_attr( $data['plan_name'] ) : ''; ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="plan_id">Plan ID *</label></th>
        <td><?php echo isset( $data['plan_id'] ) ? esc_attr( $data['plan_id'] ) : ''; ?></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td>
          <?php $tenure = isset( $data['tenure'] ) ? $data['tenure'] : '1 year'; ?>
      ...
      <tr>
        ...
        <td>
          <input
            ...
            class="small-text"
            step="0.01"
            value="<?php echo isset( $data['price'] ) ? esc_attr( $data['price'] ) : ''; ?>">
          </td>
      </tr>
      <tr>
        <th scope="row"><label for="payment_gateway_id">Payment Gateway ID</label></th>
        <td><input type="text" name="payment_gateway_id" id="payment_gateway_id" class="regular-text" value="<?php echo isset( $data['payment_gateway_id'] ) ? esc_attr( $data['payment_gateway_id'] ) : ''; ?>"></td>
      </tr>
    </table>

    <p class="submit">
      <input type="submit" class="button button-primary button-large" value="<?php echo isset( $data['id'] ) ? 'Update Subscription' : 'Add Subscription'; ?>">
